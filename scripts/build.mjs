...
const CARD_FIELDS = 'id,name,idList,due,dueComplete,idLabels,pos,closed,shortUrl';
...
const nodes = [
  ...
  { name: 'Route', type: 'n8n-nodes-base.switch', typeVersion: 3.2, position: [1120, 120],
    parameters: { mode: 'expression', numberOutputs: 2, output: "={{ $json.kind === 'notify' ? 1 : 0 }}" } },
  { name: 'Apply to Trello', type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [1340, 40], credentials: TRELLO,
  ...
  { name: 'Send reminder', type: 'n8n-nodes-base.gmail', typeVersion: 2.1, position: [1340, 220],
    webhookId: '1f0f0d3e-3c55-4b8e-9b0e-6a3f0a6f2a11',
    credentials: { gmailOAuth2: { id: '4QnrNQASnIWawPPD', name: 'Gmail account' } },
    parameters: { resource: 'message', operation: 'send', sendTo: 'tranvanthong.hd@gmail.com',
      subject: '={{ $json.subject }}', emailType: 'html', message: '={{ $json.html }}', options: { appendAttribution: false } } },
];
...
const edges = [
  ...
  link('Tick context', 'Engine'), link('Engine', 'Route'), link('Route', 'Apply to Trello', 0), link('Route', 'Send reminder', 1),
];
const connections = {};
